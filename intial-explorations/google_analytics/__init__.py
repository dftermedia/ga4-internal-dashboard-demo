...
def google_analytics(
    ...
    for query in queries:
        ...
        # Extract dimension names from dicts and find time dimension
        time_dimension = next(
            (dim.get("name") for dim in dimensions if dim.get("name") in TIME_DIMENSIONS),
            "date",  # Default to "date" if no time dimension found
        )
        # Check if time_dimension already exists in dimensions (as a dict)
        dimension_names = [dim.get("name") for dim in dimensions if isinstance(dim, dict) and "name" in dim]
        if time_dimension not in dimension_names:
            dimensions.append({"name": time_dimension})
        # Convert dimensions to list of strings (extract names from dicts)
        dimensions_list = [
            dim.get("name") if isinstance(dim, dict) and "name" in dim else dim
            for dim in dimensions
        ]
        # Convert metrics to list of strings (extract names from dicts)
        metrics_list = [
            m.get("name") if isinstance(m, dict) and "name" in m else m
            for m in query.get("metrics", [])
        ]
        # Generate resource_name if not provided
        resource_name = query.get("resource_name")
        if not resource_name:
            # Auto-generate a resource name based on dimensions and metrics
            dim_names_str = "_".join(sorted(dimensions_list[:3]))  # Use first 3 dimension names
            metric_names_str = "_".join(sorted(metrics_list[:2]))  # Use first 2 metric names
            resource_name = f"report_{dim_names_str}_{metric_names_str}"[:50]  # Limit length
